...
#   - TEMPLATE_UUID
xe_find_template() {
	local RES=$("${XE}" $LOGIN template-list name-label="$VM_TEMPLATE" params=uuid --minimal | xargs)
	if [ $? -ne 0 ]; then
...
xe_find_iso() {
    ISO_UUID=""
	local RES=$("${XE}" $LOGIN cd-list | grep ':')
	if [ $? -ne 0 ]; then
...
xe_find_net() {
	...
	local RES=$("${XE}" $LOGIN network-list name-label=$VM_NET params=uuid --minimal | xargs)
	if [ $? -ne 0 ]; then
...
#   - VM_UUID
xe_find_vm() {
	local RES=$("${XE}" $LOGIN vm-list name-label=$VM_NAME params=uuid --minimal | xargs)
	if [ $? -ne 0 ]; then
...
#   - SR_UUID
xe_find_sr() {
	local RES=$("${XE}" $LOGIN sr-list name-label="$VM_SR" params=uuid --minimal | xargs)
	if [ $? -ne 0 ]; then
        logFatal "XE Failed to list storage repositories"
	elif [ -z "$RES" ]; then
		logFatal "Could not find SR : $VM_SR"
	else
		SR_UUID=${RES%$'\r'}
...
#   - VM_UUID
xe_install_vm() {
	local cmd="${LOGIN} vm-install new-name-label=${VM_NAME} sr-uuid=${SR_UUID} --minimal"
...
xe_adjust_disk_size() {
	...
	if [ ! -z $VM_DISK ]; then
		...
		# Get disk size
		local RES=$("${XE}" $LOGIN vdi-param-get uuid="$VDI_UUID" param-name=virtual-size --minimal | xargs)
		if [ $? -ne 0 ]; then
			...
			if [ $size -ne $VM_DISK ]; then
				...
				RES=$("${XE}" $LOGIN vdi-resize uuid="$VDI_UUID" disk-size=${VM_DISK}MiB --minimal | xargs)
				if [ $? -ne 0 ]; then
...
xe_vm_shutdown() {
	...
	if [ "$cur_state" != "halted" ]; then
		RES=$("${XE}" $LOGIN vm-shutdown uuid="$VM_UUID" --minimal | xargs)
		if [ $? -ne 0 ]; then
...
xe_adjust_ram_size() {
	if [ ! -z $VM_RAM ]; then
		...
		if [[ $VM_RAM -ne $ram_min ]] || [[  $VM_RAM -ne $ram_max ]]; then
			...
			local RES=$("${XE}" $LOGIN vm-memory-set memory=${VM_RAM}MiB uuid=$VM_UUID --minimal | xargs)
			RES=${RES%$'\r'}
			if [ $? -ne 0 ]; then
...
xe_attach_iso() {
	...
	if [ ! -z $VM_ISO ]; then
		...
		local RES=$("${XE}" $LOGIN vm-cd-list uuid=$VM_UUID vbd-params=none vdi-params=uuid --multiple --minimal | xargs)
		if [ $? -ne 0 ]; then
			...
			if [ -z "$CUR_UUID" ]; then
				RES=$("${XE}" $LOGIN vm-cd-eject uuid=$VM_UUID --minimal | xargs)
				if [ $? -ne 0 ]; then
		...
		if [ -z "$CUR_UUID" ]; then
			RES=$("${XE}" $LOGIN vm-cd-insert cd-name=$VM_ISO uuid=$VM_UUID --minimal | xargs)
			if [ $? -ne 0 ]; then
				...
				# Add a new CD drive
				RES=$("${XE}" $LOGIN vm-cd-add cd-name=$VM_ISO device=$DEV_ID uuid=$VM_UUID --minimal | xargs)
				if [ $? -ne 0 ]; then
...
xe_attach_network() {
	...
	if [ ! -z $VM_NET ]; then
		...
		VIF_UUID=""
		local RES=$("${XE}" $LOGIN vm-vif-list uuid=$VM_UUID params=uuid --minimal | xargs)
		if [ $? -ne 0 ]; then
			logFatal "XE Failed to list networks"
		elif [ ! -z "$RES" ]; then
			RES=${RES%$'\r'}
			...
			for i in ${RES//,/ }; do
				local RES2=$("${XE}" $LOGIN vif-list uuid=$i params=network-uuid --minimal | xargs)
				RES2=${RES2%$'\r'}
				if [ $? -ne 0 ]; then
					logFatal "XE Failed to list VIFs"
				elif [ -z "$RES2" ]; then
					logFatal "We should not find a VIF with no networks"
				elif [ "${RES2}" == "${NET_UUID}" ]; then
		...
		if [ -z "$VIF_UUID" ]; then
			...
			# Create the VIF
			RES=$("${XE}" $LOGIN vif-create vm-uuid=$VM_UUID device=$DEV_ID network-uuid=$NET_UUID mac=random --minimal | xargs)
			if [ $? -ne 0 ]; then
...
xe_start() {
	...
	if [ "$cur_state" != "running" ]; then
		RES=$("${XE}" $LOGIN vm-start uuid="$VM_UUID" --minimal | xargs)
		if [ $? -ne 0 ]; then
			logFatal "XE Failed to shutdown ${VM_UUID}"
		elif [ ! -z "$RES" ]; then
			logFatal "Non-Empty response. Did we fail to shutdown state? ($RES)"
...
xe_find_disk() {
	...
	VDI_UUID=""
	local RES=$("${XE}" $LOGIN vm-disk-list uuid=$VM_UUID vdi-params=uuid,name-label vbd-params=none | grep ': ')
	if [ $? -ne 0 ]; then
...
xe_get_vm_param() {
	...
	local RES=$("${XE}" $LOGIN vm-param-get uuid=$VM_UUID param-name=$1 --minimal | xargs)
	PARAM=${RES%$'\r'}
	if [ $? -ne 0 ]; then
...
xe_set_vm_param() {
	...
	local RES=$("${XE}" $LOGIN vm-param-set uuid=$VM_UUID $1=$2 --minimal | xargs)
	PARAM=${RES%$'\r'}
	if [ $? -ne 0 ]; then
...
xe_upload_file() {
	...
	case $proto in
		file://*) # UNC/SMB
			...
			local res=$(eval $cmd)
			if [ $? -ne 0 ]; then
				logFatal "SMB Failed to list files in: ${uri}/${path}"
			elif [ -z "$res" ]; then
				logFatal "We should not receive an empty response"
			elif [[ "$(echo ${res%$'\r'} | xargs)" =~ $not_found_regex ]]; then
				res=$(echo ${res%$'\r'} | xargs)
				logInfo "File does not exists. Proceed with upload..."
				logTrace "$res"
			elif [[ "$(echo ${res%$'\r'} | xargs)" =~ $found_regex ]]; then
				res=$(echo ${res%$'\r'} | xargs)
...
xe_find_vbd_device_id() {
	...
	local RES=$(echo ${PARAM//;/}) # Seperate by removing the semicolon
...
xe_create_disk() {
	...
	if [ ! -z $VM_DISK ]; then
		...
		if [ -z $VDI_UUID ]; then
			local vdi=""
			RES=$("${XE}" $LOGIN vdi-create sr-uuid=$SR_UUID name-label=${VM_NAME}_hdd virtual-size=${VM_DISK}MiB --minimal | xargs)
			if [ $? -ne 0 ]; then
			...
			xe_find_vbd_device_id;
			RES=$("${XE}" $LOGIN vbd-create vm-uuid=$VM_UUID device=$DEV_ID vdi-uuid=$vdi type=Disk mode=RW --minimal | xargs)
			if [ $? -ne 0 ]; then
...
xe_rename_new_disks() {
	...
	local RES=$("${XE}" $LOGIN vm-disk-list uuid=$VM_UUID vdi-params=uuid vbd-params=none --minimal | xargs)
	if [ $? -ne 0 ]; then
		...
		echo "Res: ${RES}"
		RES=${RES%$'\r'}
		local nb_disks=$(echo "${RES//,/ }" | wc -w)
		logTrace "${nb_disks} disks were created"
		if [ $nb_disks -eq 1 ]; then
			RES=$("${XE}" $LOGIN vdi-param-set uuid=$RES name-label=${VM_NAME}_hdd --minimal | xargs)
			if [ $? -ne 0 ]; then
			...
			local disk_count=0
			for i in ${RES//,/ }; do
				local cmd="${LOGIN} vdi-param-set uuid=$i name-label=${VM_NAME}_hdd"
...
xe_add_tags() {
	if [ ! -z $VM_TAGS ]; then
		...
		local cur_tags=$(echo $PARAM | xargs)

		for i in ${VM_TAGS//,/ }; do
			local req_tag=${i%$'\r'}
			local found=0
			for j in ${cur_tags//,/ }; do
				local cur_tag=$(echo ${j%$'\r'} | xargs)
				if [ $i == $j ]; then
					found=1
					logInfo "Tag \"$j\" is already present"
					break;
				else
					logTrace "Ignoring tag: $j"
				fi
			done

			if [[ $found -eq 0 ]]; then
				local RES=$("${XE}" $LOGIN vm-param-add uuid=$VM_UUID param-name=tags param-key=$i --minimal | xargs)
				RES=${RES%$'\r'}
				if [ $? -ne 0 ]; then
					logFatal "XE set tag \"$i\" on VM: $VM_NAME"
				elif [ ! -z "$RES" ]; then
					logFatal "Non-empty response. Did we fail to set tag $i: ${RES}"
				else
					logTrace "Tag added: $i"
				fi
			fi
		done
	fi
